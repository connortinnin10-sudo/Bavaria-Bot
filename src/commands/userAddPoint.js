...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("user_add_point")
    .setDescription("Add promotion points to one or more members (1-5)")
    .addStringOption((opt) =>
      opt.setName("users").setDescription("Tag one or more members").setRequired(true)
    )
    .addIntegerOption((opt) =>
      opt.setName("amount").setDescription("Points to add per member (1-5)").setRequired(true).setMinValue(1).setMaxValue(5)
    ),
  ...
  async execute(interaction) {
    ...
    const lines = [];
    if (updated.length) {
      lines.push(`✅ **+${amount}** point${amount === 1 ? "" : "s"} to:`);
