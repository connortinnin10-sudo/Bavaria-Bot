...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("user_promote")
    .setDescription("Promote a regiment member to a new rank")
    .addUserOption((opt) =>
      opt.setName("user").setDescription("The member to promote").setRequired(true)
    )
    .addStringOption((opt) =>
      opt
        .setName("promoted_to")
  ...
  async execute(interaction) {
    ...
    const newRank      = interaction.options.getString("promoted_to");
    ...
    if (PROTECTED_RANKS.has(currentRank)) {
      return interaction.editReply({
        content: `**${targetUser.username}** holds the rank of **${currentRank}** and cannot be promoted through this command.`,
      });
    }
    ...
    // Swap Discord rank roles — remove all rank roles then add new one
    ...
    return interaction.editReply({
      content: `✅ **${targetUser.username}** has been promoted from **${currentRank || "Unknown"}** to **${newRank}**.`,
    });
  },
};
