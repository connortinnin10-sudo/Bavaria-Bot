...
3. **Discord roles (built):** strip every role a member holds EXCEPT the keep-list, then add the reserve roles.
   - Keep-list = `RESERVE_KEEP_ROLE_IDS` in `src/permissions.js` = `PROTECTED_ROLE_IDS` + the 6 enlisted rank roles (`ENLISTED_RANK_ROLE_IDS`, Conscript→Caporal-Fourrier). Also skipped by the command loop: **earned honour roles** (`HONOUR_ROLE_IDS` from `src/honoursSheet.js` — every medal/nobility/veneration/grandbattle role, shared with `honours_sync` so the two never drift), managed roles (booster/integration), and `@everyone`.
   - The sweep therefore removes the regiment, company, corps/army, Donauwörth, department, specialization, staff/command-access, and **officer-rank** roles in one pass — but never medals/honours.
   - Then add `ROLE_BAVARIAN_RESERVES` (`1193239194529714382`, the repurposed old-regiment "merc" role) plus the two `RESERVE_EXTRA_ROLE_IDS` grouping roles (`1203775751577407489` "Mercenary", `1234315658406269018` "Réserves du Premier Corps") to **everyone**, and `ROLE_BAVARIA_VETERAN` (`1530414516330827826`) additionally to **veterans**. All hardcoded in `permissions.js` per the Railway pitfall.
   - A veteran whose rank was capped to `Caporal-Fourrier` (a demoted officer — the sweep stripped their officer rank role) is (re)assigned the `RANK_ROLE_CAPORAL_FOURRIER` Discord role so Discord matches the locked sheet rank. Non-officer veterans keep their real rank role via the keep-list.
...
**Return paths strip the reserve roles again** (so a member coming back isn't left flagged as reserve): `/transfer_company`'s veteran-return branch removes `ROLE_BAVARIAN_RESERVES` + `ROLE_BAVARIA_VETERAN`, and **every** `/transfer_company` and `/user_enlist` run unconditionally strips the two `RESERVE_EXTRA_ROLE_IDS` grouping roles (no-op if absent); `/user_enlist`'s mercenary re-enlist also removes `ROLE_BAVARIAN_RESERVES`.
...
- **LOA storage:** LOAs live in the **`LOA`** tab (GID `699157387`), set via `ACCOUNTABILITY_TAB` in `src/sheets.js` (the old empty "Accountability" tab is no longer used). Layout is columns **A:G** = `[Discord ID, roster tab name, row number, leave date, return date, reason, officer ID]`, no header, dates **DD/MM/YY** (`parseDate` reads day-first). `/user_loa` appends a row; `/user_loa_remove` clears it. The midnight-EST job (`clearExpiredAccountabilities` → `runDailyCheck`/`scheduleMidnightCheck` in `index.js`, which also runs once on every startup) activates LOAs whose leave date is today (sets the roster's column-J checkbox TRUE, DMs the member) and expires LOAs whose return date has passed (clears the checkbox, clears the row, DMs the member). It locates the member's roster row via `findUser`, not the stored tab/row columns.
